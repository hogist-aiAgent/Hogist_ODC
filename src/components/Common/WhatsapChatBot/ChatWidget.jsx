...
import avatarIcon from "../../../assets/CompanyLogo/avatarLogo.png";
...
const ChatWidget = () => {
  ...
  const setIsOpen = (val) => (val ? openChat() : closeChat());
  ...
  const localChat = JSON.parse(safeStorage.getItem("messgae") || "[]");
  let id = safeStorage.getItem("chat_id");
  ...
  useEffect(() => {
    if (localChat.length) {
      ...
      safeStorage.setItem("messgae", JSON.stringify(welcomeMessage));
    }
  }, []);

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    safeStorage.setItem("messgae", JSON.stringify(messages));
  }, [messages]);
  ...
  const startNewChat = () => {
    ...
    setInput("");
    false;
    safeStorage.removeItem("chat_id");
    safeStorage.setItem("messgae", JSON.stringify(welcomeMessage));
  };
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      safeStorage.setItem("chat_id", res.data.chat_id);
