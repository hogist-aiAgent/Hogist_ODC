import img1 from '../assets/menu/chosseRestaurent/img1.jpg';
import img2 from '../assets/menu/chosseRestaurent/img2.jpg';

const restaurants = [
  {
    id: 1,
    name: 'Aswathy Caterers',
    area: 'Madhavaram',
    matchKeywords: ['madhavaram','Madhavaram'],
    fssai: '22419246000629',
    tags: ['South Indian', 'North Indian'],
    rating: '4.3',
    ribbon: 'TOP RATED',
    img: img1,
    isVeg: true,
    isNonVeg: true,
  },
  {
    id: 2,
    name: 'CES Catering Service',
    area: 'Choolaimedu',
    matchKeywords: ['choolaimedu', 'chozhiimedu','Choozhaimedu',],
    fssai: '13422000001516',
    tags: ['South Indian', 'North Indian'],
    rating: '4.1',
    ribbon: 'POPULAR',
    img: img2,
    isVeg: true,
    isNonVeg: true,
  },
  {
    id: 3,
    name: 'Meenakshi Catering',
    area: 'KK Nagar',
    matchKeywords: ['kk nagar', 'k.k. nagar', 'kk. nagar','kknagar','KK Nagar'],
    fssai: '22424545000557',
    tags: ['South Indian'],
    rating: '4.5',
    ribbon: 'TOP RATED',
    img: img1,
    isVeg: true,
    isNonVeg: false,
  },
  {
    id: 4,
    name: 'Sri Balaji Catering Service',
    area: 'Valasaravakkam',
    matchKeywords: ['valasaravakkam', 'Valasaravakkam', 'valasaravakam'],
    fssai: '22419530000140',
    tags: ['South Indian'],
    rating: '4.0',
    ribbon: 'NEW',
    img: img2,
    isVeg: true,
    isNonVeg: false,
  },
  {
    id: 5,
    name: 'Saravana Bhavan Catering',
    area: 'T Nagar',
    matchKeywords: ['t nagar', 'thyagaraya nagar', 'theagaraya nagar','T. Nagar','TNagar','Thyagaraya Nagar','Theagarayanagar'],
    fssai: '22411145000821',
    tags: ['South Indian', 'North Indian'],
    rating: '4.6',
    ribbon: 'TOP RATED',
    img: img1,
    isVeg: true,
    isNonVeg: false,
  },
  {
    id: 6,
    name: 'Green Leaf Caterers',
    area: 'Adyar',
    matchKeywords: ['adyar', 'Adyar','adayar'],
    fssai: '22417745000934',
    tags: ['South Indian'],
    rating: '3.2',
    ribbon: 'POPULAR',
    img: img2,
    isVeg: false,
    isNonVeg: true,
  },
  {
    id: 7,
    name: 'Spice Route Catering',
    area: 'Velachery',
    matchKeywords: ['velachery','Velachery','velacherry'],
    fssai: '22413345001127',
    tags: ['North Indian', 'Chinese'],
    rating: '4.0',
    ribbon: 'NEW',
    img: img1,
    isVeg: false,
    isNonVeg: true,
  },
  {
    id: 8,
    name: 'Annapoorna Catering Services',
    area: 'Anna Nagar',
    matchKeywords: ['anna nagar','Anna Nagar','annanagar'],
    fssai: '22412245001338',
    tags: ['South Indian', 'North Indian'],
    rating: '4.4',
    ribbon: 'TOP RATED',
    img: img2,
    isVeg: true,
    isNonVeg: true,
  },
  {
    id: 9,
    name: 'Tambaram Thali Caterers',
    area: 'Tambaram',
    matchKeywords: ['tambaram','Tambaram','tambarum','tambaram east','tambaram west'],
    fssai: '22419945001449',
    tags: ['South Indian'],
    rating: '3.9',
    ribbon: 'NEW',
    img: img1,
    isVeg: true,
    isNonVeg: false,
  },
  {
    id: 10,
    name: 'OMR Feast Catering',
    area: 'Sholinganallur',
    matchKeywords: ['sholinganallur', 'omr', 'perungudi', 'thoraipakkam'],
    fssai: '22418845001552',
    tags: ['North Indian', 'Chinese'],
    rating: '4.1',
    ribbon: 'POPULAR',
    img: img2,
    isVeg: true,
    isNonVeg: true,
  },
  {
    id: 11,
    name: 'Porur Grand Caterers',
    area: 'Porur',
    matchKeywords: ['porur', 'Porur', 'porur road', 'porur main road'],
    fssai: '22416645001663',
    tags: ['South Indian', 'North Indian'],
    rating: '4.3',
    ribbon: 'TOP RATED',
    img: img1,
    isVeg: true,
    isNonVeg: true,
  },
  {
    id: 12,
    name: 'Mylapore Mess Catering',
    area: 'Mylapore',
    matchKeywords: ['mylapore', 'Mylapore'],
    fssai: '22414445001774',
    tags: ['South Indian'],
    rating: '4.5',
    ribbon: 'TOP RATED',
    img: img2,
    isVeg: true,
    isNonVeg: false,
  },
  {
    id: 13,
    name: 'Mess Catering',
    area: 'Mylapore',
    matchKeywords: ['mylapore', 'Mylapore'],
    fssai: '22414445001774',
    tags: ['South Indian'],
    rating: '4.2',
    ribbon: 'TOP RATED',
    img: img2,
    isVeg: true,
    isNonVeg: false,
  },
];

export default restaurants;


export const filterNearbyRestaurants = (locationText) => {
  if (!locationText) return [];
  const lower = locationText.toLowerCase();
  return restaurants.filter((r) =>
    r.matchKeywords.some((keyword) => lower.includes(keyword))
  );
};